...
built for a *different* scenario (see below).
...
- **`docs/PRD.md` Sec. 8.9 ("Super Admin's day")**: "Reviews new tenant signups and onboarding
  status" — implies Super Admin has *visibility* into new signups, and possibly a review/approval
...
- **`docs/PHASES.md`**: this is the most telling gap. Phase 1 only creates the `tenants` /
  ...
  *already-provisioned* users plus the Parent `/access-pending` state — it does **not** scope a
...
4. `src/lib/super-admin.functions.ts` — the only file that touches `tenants` — has a stats/list query
   and an `UPDATE` (for status changes on an *existing* tenant). **There is no tenant `INSERT`
...
6. `src/routes/access-pending.tsx` is hardcoded copy for **"Parent phone not yet linked to a
   ...
   "contact your hostel administration" — which is nonsensical for someone who *is* the hostel
...
- Needs: a genuine `/access-pending`-style state *for this case specifically* (distinct from the
