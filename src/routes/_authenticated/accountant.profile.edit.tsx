...
export const Route = createFileRoute("/_authenticated/accountant/profile/edit")({
  head: () => ({ meta: [{ title: "Edit Profile — Hostylia" }] }),
  component: AccountantProfileEditPage,
});

function AccountantProfileEditPage() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      const parsed = accountantProfileEditSchema.safeParse({ fullName, phone });
      if (!parsed.success) {
    ...
    onSuccess: () => {
      ...
      navigate({ to: "/accountant/profile" });
    },
    onError: (e) => {
