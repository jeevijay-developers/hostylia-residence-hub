    ...
    <div className="space-y-4">
      ...
      {(menusQ.data ?? []).map((m) => (
        <MenuCard
          key={m.id}
          menu={m}
          submittedRating={submittedMap.get(m.id)}
          student={studentQ.data}
          kycComplete={kycComplete}
          onSubmitted={() => {
            qc.invalidateQueries({ queryKey: ["my-mess-feedback"] });
          }}
        />
      ))}
...
function MenuCard({
  ...
  menu: {
    ...
    meal: string;
    title: string | null;
    mess_menu_items?: Array<{ item_name: string }>;
  };
  submittedRating: number | undefined;
  student: { id: string; tenant_id: string; property_id: string } | null | undefined;
  kycComplete: boolean;
  onSubmitted: () => void;
}) {
