  ...
  return (
    <BrowserFrame url="app.hostylia.com/occupancy">
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        {[
          ...
          <div key={s.l} className="rounded-xl border border-dark-border bg-white/5 p-3">
            <div
              className={`text-xl font-extrabold ${s.t === "teal" ? "text-soft-teal" : s.t === "gold" ? "text-gold" : "text-white"}`}
            >
              {s.v}
            </div>
      ...
      <div className="mt-4 grid grid-cols-6 gap-1.5">
        {Array.from({ length: 24 }).map((_, i) => {
          ...
          return (
            <div
              key={i}
              className={`flex h-10 items-center justify-center rounded-lg border text-[10px] font-bold text-white ${color}`}
            >
              {i + 301}
            </div>
          );
      ...
      <div className="mt-3 flex items-center gap-3 text-[10px] text-soft-grey">
        <span className="flex items-center gap-1">
          <span className="h-2 w-2 rounded-sm bg-soft-teal" /> Occupied
        </span>
        <span className="flex items-center gap-1">
          <span className="h-2 w-2 rounded-sm bg-gold" /> On notice
        </span>
        <span className="flex items-center gap-1">
          <span className="h-2 w-2 rounded-sm border border-dark-border" /> Vacant
        </span>
      </div>
    </BrowserFrame>
  );
  ...
  return (
    <BrowserFrame url="app.hostylia.com/finance">
      <div className="flex items-center justify-between">
        ...
        <div className="rounded-lg border border-dark-border bg-white/5 px-2.5 py-1.5 text-[10px] text-soft-grey">
          FY 25-26
        </div>
      </div>
      <div className="mt-4 flex h-28 items-end gap-1.5">
        {months.map((h, i) => (
          <div
            key={i}
            className="flex-1 rounded-t-md bg-gradient-to-t from-[color:var(--brand-blue)] to-soft-teal"
            style={{ height: `${h}%` }}
          />
        ))}
      </div>
...
export function MockComplaintBoard() {
  const items = [
    {
      t: "Leaking tap — Room 214",
      s: "Resolved",
      c: "text-soft-teal",
      b: "border-soft-teal/40 bg-soft-teal/10",
    },
    { t: "Wi-Fi slow — Block B", s: "In Progress", c: "text-gold", b: "border-gold/40 bg-gold/10" },
    {
      t: "AC service — Room 408",
      s: "Assigned",
      c: "text-[color:var(--brand-blue)]",
      b: "border-[color:var(--brand-blue)]/40 bg-[color:var(--brand-blue)]/10",
    },
    ...
    <BrowserFrame url="app.hostylia.com/complaints">
      ...
      <div className="mt-4 space-y-2">
        {items.map((i) => (
          <div
            key={i.t}
            className="flex items-center justify-between rounded-xl border border-dark-border bg-white/5 px-3 py-2.5"
          >
            <div className="text-xs font-semibold text-white">{i.t}</div>
            <div className={`rounded-full border px-2 py-0.5 text-[10px] font-bold ${i.b} ${i.c}`}>
              {i.s}
            </div>
          </div>
        ))}
    ...
    <PhoneFrame>
      ...
      <div className="mt-3 space-y-2">
        {[
          ...
          <div
            key={r.t}
            className="flex items-center justify-between rounded-lg border border-dark-border bg-white/5 px-2.5 py-2"
          >
            <div className="flex items-center gap-2">
              <span className="grid h-7 w-7 place-items-center rounded-md bg-soft-teal/15 text-soft-teal">
                <r.i size={14} />
              </span>
              <div>
    ...
    <PhoneFrame>
      ...
      <div className="mt-3 rounded-xl border border-dark-border bg-white/5 p-3">
        {[
          ...
          <div
            key={r.l}
            className="flex items-center justify-between border-b border-dark-border py-1.5 last:border-0 text-[11px] text-white"
          >
  ...
  return (
    <PhoneFrame>
      <div className="flex items-center justify-between">
        ...
        <div className="grid h-8 w-8 place-items-center rounded-full bg-white/5 text-white">
          <Bell size={14} />
        </div>
      </div>
      ...
      <div className="mt-3 space-y-2">
        {[
          ...
          <div
            key={it.t}
            className="flex items-center gap-3 rounded-lg border border-dark-border bg-white/5 px-3 py-2"
          >
            <span className="grid h-7 w-7 place-items-center rounded-md bg-soft-teal/15 text-soft-teal">
              <it.i size={14} />
            </span>
    ...
    <BrowserFrame url="app.hostylia.com/ai">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {[
          ...
          {
            t: "Top performing block",
            v: "Block A · 98%",
            d: "Replicate playbook",
            c: "text-soft-teal",
          },
        ].map((i) => (
          <div key={i.t} className="rounded-xl border border-dark-border bg-white/5 p-3">
            ...
            <div className="mt-1 inline-flex items-center gap-1 text-[10px] font-semibold text-white">
              {i.d} <ArrowUpRight size={11} />
            </div>
          </div>
        ))}
      </div>
    ...
    <BrowserFrame url="app.hostylia.com/security">
      ...
      <div className="mt-3 overflow-hidden rounded-xl border border-dark-border">
        {rows.map((r, i) => (
          <div
            key={i}
            className="flex items-center justify-between border-b border-dark-border bg-white/5 px-3 py-2 last:border-0"
          >
            <div className="flex items-center gap-2">
              <span
                className={`grid h-7 w-7 place-items-center rounded-full text-[10px] font-bold ${
                  r.s === "alert"
                    ? "bg-[#E55353]/20 text-[#E55353]"
                    : r.s === "visitor"
