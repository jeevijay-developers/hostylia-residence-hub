import { Bed, IndianRupee, MessageSquareWarning, ClipboardCheck, BellRing, TrendingUp } from "lucide-react";

const rooms = [
  { n: "101", s: "occupied" }, { n: "102", s: "occupied" }, { n: "103", s: "vacant" }, { n: "104", s: "maintenance" },
  { n: "105", s: "occupied" }, { n: "106", s: "alert" }, { n: "107", s: "occupied" }, { n: "108", s: "vacant" },
  { n: "109", s: "occupied" }, { n: "110", s: "occupied" }, { n: "111", s: "vacant" }, { n: "112", s: "occupied" },
] as const;

const statusStyle: Record<string, string> = {
  vacant: "bg-[color-mix(in_oklab,var(--trust-green)_25%,transparent)] text-[color:var(--trust-green)] border-[color-mix(in_oklab,var(--trust-green)_40%,transparent)]",
  occupied: "bg-[color-mix(in_oklab,var(--brand-blue)_28%,transparent)] text-white border-[color-mix(in_oklab,var(--brand-blue)_55%,transparent)]",
  maintenance: "bg-[color-mix(in_oklab,var(--gold)_28%,transparent)] text-gold border-[color-mix(in_oklab,var(--gold)_55%,transparent)]",
      ...
      <div className="glass-panel rounded-3xl p-5 shadow-2xl">
        ...
        <div className="mt-5 rounded-2xl border border-dark-border bg-[color-mix(in_oklab,var(--navy)_70%,transparent)] p-4">
          <div className="mb-3 flex items-center justify-between">
            <div>
              <div className="text-xs font-semibold uppercase tracking-widest text-soft-grey">Block A · Floor 1</div>
          ...
          <div className="grid grid-cols-6 gap-2">
            {rooms.map((r) => (
              <div key={r.n} className={`flex flex-col items-center justify-center gap-1 rounded-lg border p-2 ${statusStyle[r.s]}`}>
                <Bed size={14} />
        ...
        <div className="mt-4 flex items-start gap-3 rounded-xl border border-dark-border bg-[color-mix(in_oklab,var(--indigo-deep)_60%,transparent)] p-3">
          ...
          <div className="min-w-0 text-xs">
            ...
            <div className="truncate text-soft-grey">Rahul S. exited campus at 6:42 PM · Gate Pass #2148</div>
          </div>
          <span className="ml-auto rounded-full bg-[color-mix(in_oklab,var(--trust-green)_25%,transparent)] px-2 py-0.5 text-[10px] font-bold text-[color:var(--trust-green)]">Delivered</span>
        </div>
      </div>
    </div>
  );
...
function Stat({ icon: Icon, label, value, tone }: { icon: any; label: string; value: string; tone: "teal" | "green" | "blue" | "gold" }) {
  const colors = {
    teal: "text-soft-teal",
  ...
  return (
    <div className="rounded-xl border border-dark-border bg-[color-mix(in_oklab,var(--navy)_70%,transparent)] p-3">
      <div className={`flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-widest ${colors}`}>
        <Icon size={12} /> {label}
      </div>
