...
//  4) SMS → prefer MSG91 Flow when MSG91_AUTH_KEY is set and templateKey is mapped;
//     else Twilio free-text if configured. WHATSAPP → Twilio. EMAIL → Resend.
//     Missing provider → { ok:false, error_code:"PROVIDER_NOT_CONFIGURED" }.
//     Configured path queues attempt, responds PENDING, delivers via EdgeRuntime.waitUntil.
//
...
async function dispatchAndRespond(
  ...
  if (channel === "SMS") {
    if (hasMsg91 && msg91Mapped) {
      provider = "msg91";
      configured = true;
    } else if (twilioSid && twilioTok && twilioFrom) {
      provider = "twilio";
      configured = true;
    } else {
      provider = hasMsg91 ? "msg91" : "twilio";
      configured = false;
    }
  }
  if (channel === "WHATSAPP") {
    ...
    const hint =
      channel === "EMAIL"
        ...
        : channel === "WHATSAPP"
          ...
          : hasMsg91 && !msg91Mapped
            ? `MSG91 mapping for templateKey=${templateKey}`
            : "MSG91_AUTH_KEY (preferred) or TWILIO_* for SMS";
    await admin
      .from("notification_attempts")
      .update({
        status: "FAILED",
    ...
    return json(
      {
        ...
        notification_id: notifId,
        error_code: "PROVIDER_NOT_CONFIGURED",
        message: `${channel} not configured — add ${hint}.`,
      },
      200,
    );
  }
  ...
  const deliver = async () => {
    try {
      ...
      if (channel === "SMS") {
        if (provider === "msg91") {
          if (!recipient.phone) throw new Error("missing_phone");
          const sent = await sendMsg91TemplateSms(templateKey, recipient.phone, variables);
        ...
        const from = `whatsapp:${twilioWaFrom}`;
        const body = renderTemplate(templateKey, variables);
        const res = await fetch(
          `https://api.twilio.com/2010-04-01/Accounts/${twilioSid}/Messages.json`,
          {
